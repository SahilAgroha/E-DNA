...
const Chatbot = () => {
    ...
    const handleSendMessage = async (e) => {
        e.preventDefault();
        if (!input.trim() || !selectedAnalysisId) return;
        ...
        try {
            ...
            The current analysis data is for Analysis ID: ${currentAnalysis?.id}. 
            Key metrics:
            ...
            - Dominant Kingdom (Sample_Pond_A): ${currentAnalysis?.diversity_data?.sample_summary?.Sample_Pond_A?.dominant_kingdom}
            - Shannon Diversity (Sample_Lake_B): ${currentAnalysis?.diversity_data?.alpha_diversity?.shannon_diversity?.Sample_Lake_B}
            ...
            setChatHistory(prev => [...prev, { role: 'bot', content: 'Sorry, I am unable to connect to the service. Please try again later.' }]);
    ...
    const handleAnalysisChange = (event) => {
        ...
        if (id) {
            dispatch(fetchAnalysisAsync(id));
